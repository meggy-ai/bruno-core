...
class BrunoDiscordBot(BaseBrunoAssistant):
    ...
    def __init__(self, token: str, config: BrunoConfig):
        ...
        # Store token for bot.run()
        self.token = token
        
        # Audio transcriber for voice messages
        transcription_engine = config.get('speech.transcription_engine', 'auto')
        model_size = config.get('speech.faster_whisper.model', 'small')
        try:
            ...
            logger.warning(f"⚠️  Could not initialize transcriber: {e}")
    ...
    async def music_update_task(self):
        """Background task to update music playback (auto-advance playlist)."""
        if self.music_manager:
            self.music_manager.update_playback()
    
    @music_update_task.before_loop
    async def before_music_update_task(self):
        """Wait until bot is ready before starting music updates."""
    ...
    def _cleanup_resources(self) -> None:
        ...
        try:
            ...
            if self.music_update_task.is_running():
                self.music_update_task.cancel()
    ...
    def _should_enable_tts(self) -> bool:
        ...
        return self.config.get('discord.local_actions.enable_tts', False)
    ...
    async def _send_timer_notification(self, channel, message: str):
        ...
        try:
            ...
            logger.info(f"✅ Sent timer notification to Discord: {message}")
        except Exception as e:
    ...
    def _register_handlers(self):
        ...
        async def on_ready():
            ...
            # Initialize common components (LLM, memory, abilities)
            try:
                logger.info("🔧 Setting up common components...")
                if await self._setup_common_components():
            ...
            # Start background music update task
            if not self.music_update_task.is_running():
                self.music_update_task.start()
    ...
    def _register_slash_commands(self):
        ...
        @music_group.command(name="next", description="Skip to next song in playlist")
        async def music_next(interaction: discord.Interaction):
            await interaction.response.defer()
            ...
            await interaction.followup.send(response.get('text', 'Skipped to next track'))
