...
class CustomLLM(LLMInterface):
    ...
    async def generate(self, messages: list[Message], **kwargs) -> str:
        ...
        last_message = next(
            (msg.content for msg in reversed(messages) if msg.role == MessageRole.USER), "Hello"
        )
...
async def main():
    ...
    advanced_llm = AdvancedCustomLLM(
        model_name="advanced-demo", rate_limit=10, max_retries=2  # 10 requests per minute
    )
    ...
    for i in range(3):
        ...
        response = await assistant2.process_message(
            message=msg, user_id="demo-user", conversation_id="demo-conv"
        )
