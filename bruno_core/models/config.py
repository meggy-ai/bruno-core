...
class MemoryConfig(BaseModel):
    ...
    max_messages: int = Field(
        default=20, ge=1, le=1000, description="Max messages in context"
    )
    compression_threshold: int = Field(
        default=50, ge=1, description="Compression trigger threshold"
    )
    stm_threshold: float = Field(
        default=0.7, ge=0.0, le=1.0, description="STM relevance threshold"
    )
    ltm_threshold: float = Field(
        default=0.8, ge=0.0, le=1.0, description="LTM importance threshold"
    )
    enable_embeddings: bool = Field(
        default=True, description="Generate embeddings"
    )
...
class AssistantConfig(BaseModel):
    ...
    event_handlers: List[str] = Field(
        default_factory=list, description="Event handlers"
    )
    middleware: List[str] = Field(default_factory=list, description="Middleware")
    additional_params: Dict[str, Any] = Field(
        default_factory=dict, description="Additional params"
    )
